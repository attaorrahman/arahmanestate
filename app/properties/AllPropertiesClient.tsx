...
function applyFilters(list: Property[], filters: Filters): Property[] {
  ...
  if (filters.emirate !== 'all') {
    result = result.filter((p) => p.emirate_slug === filters.emirate);
  }
...
export default function AllPropertiesClient({ initialProperties, emirates }: Props) {
  ...
  const [properties, setProperties] = useState<Property[]>(
    () => applyFilters(initialProperties, { emirate: 'all', priceType: 'all', propertyType: 'all', sort: 'featured' })
  );
  ...
  const updateFilter = (key: keyof Filters, value: string) => {
    ...
    debounceTimer.current = setTimeout(() => {
      setProperties(applyFilters(initialProperties, next));
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative">
              <select
                ...
                onChange={(e) => updateFilter('emirate', e.target.value)}
                className={selectClass}
              >
                <option value="all">{t('props.all_emirates')}</option>
                {emirates.map((e) => (
            ...
            <div className="relative">
              ...
              >
                ...
                <option value="newest">{t('props.newest_first')}</option>
