...
export default function OurTeam() {
  return (
    <section className="py-24 bg-[#ffffff] relative overflow-hidden" id="team">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <span className="text-[#C9A84C] font-body text-xs tracking-[0.25em] uppercase font-medium">
              The People Behind LuxEstate
            </span>
            <div className="h-px w-10 bg-[#C9A84C]" />
          </div>
          <h2 className="font-display text-4xl sm:text-5xl font-bold text-gray mb-4">
            Meet Our Team
          </h2>
          <p className="font-body text-gray-400 text-lg max-w-2xl mx-auto">
            Our specialists bring hands-on market experience to every client relationship — whether you are buying, renting, or investing across the UAE.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 max-w-5xl mx-auto">
          {team.map((member, i) => (
            <div
              key={member.name}
              className="group bg-black/5 border border-black/10 rounded-sm overflow-hidden backdrop-blur-sm hover:border-[#C9A84C]/30 transition-all duration-400 animate-fade-in-up"
            ...
            >
              ...
              <div className="flex gap-6 p-7 pb-5">
                <div className="relative shrink-0">
                  <div className="w-24 h-24 rounded-sm overflow-hidden border-2 border-[#C9A84C]/40">
                    <img
                      src={`${member.photo}?auto=compress&cs=tinysrgb&w=200&h=200&fit=crop&crop=face`}
                      alt={member.name}
                ...
                <div className="min-w-0">
                  <h3 className="font-display text-black text-xl font-bold mb-0.5">
                    {member.name}
                  </h3>
                  <p className="font-body text-[#C9A84C] text-sm font-medium mb-1">
                    {member.role}
                  </p>
                  <span className="inline-block bg-[#C9A84C]/15 text-[#C9A84C] text-xs font-body font-semibold px-2.5 py-1 rounded-sm border border-[#C9A84C]/20">
                    {member.experience}
                  </span>
                    ...
                    <div className="flex gap-0.5">
                      {Array.from({ length: 5 }).map((_, j) => (
                        <Star
                          key={j}
                          size={11}
                          className={j < Math.round(member.rating) ? 'text-[#C9A84C] fill-[#C9A84C]' : 'text-gray-600 fill-gray-600'}
                        />
                      ))}
                    </div>
                    <span className="font-body text-gray-500 text-xs">{member.rating} rating</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 divide-x divide-white/8 mx-7 my-4">
                <div className="pr-4 text-center">
                  ...
                  <p className="font-body text-gray-500 text-xs">Deals Closed</p>
                </div>
              ...
              <div className="px-7 pb-7 grid grid-cols-2 gap-3">
                <a
                  ...
                  className="flex items-center justify-center gap-2 bg-[#C9A84C] hover:bg-[#b8963f] text-black font-body text-sm font-semibold py-2.5 rounded-sm transition-colors"
                >
                  <Phone size={14} />
                  Call Now
                </a>
                <a
                  ...
                  className="flex items-center justify-center gap-2 border border-white/15 hover:border-[#C9A84C] text-darkgray-300 hover:text-[#C9A84C] font-body text-sm font-medium py-2.5 rounded-sm transition-all"
                >
                  <Mail size={14} />
                  Email
                </a>
